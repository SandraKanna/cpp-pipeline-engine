...
namespace cpe {
	TEST(LineDelimitation, EmitsCompleteLinesWhenFinal) {
		...
		auto result = delimiter.delimit(to_bytes("ab\ncde\nef\n"), true);

		ASSERT_TRUE(result.has_value());
		const auto& records = result.value();
		ASSERT_EQ(records.size(), 3U);
		EXPECT_EQ(records[0], to_bytes("ab"));
		EXPECT_EQ(records[1], to_bytes("cde"));
		EXPECT_EQ(records[2], to_bytes("ef"));
	}
	...
	TEST(LineDelimitation, DoesNotEmitIncompleteTail) {
		...
		auto first = delimiter.delimit(to_bytes("ab\nc\nde"), false);
		ASSERT_TRUE(first.has_value());
		ASSERT_EQ(first.value().size(), 2U);
		EXPECT_EQ(first.value()[0], to_bytes("ab"));
		EXPECT_EQ(first.value()[1], to_bytes("c"));
	}
	...
	TEST(LineDelimitation, ReconstructsLineSplitAcrossCalls) {
		...
		auto second = delimiter.delimit(to_bytes("fgh\n"), true);
		ASSERT_TRUE(second.has_value());
		ASSERT_EQ(second.value().size(), 1U);
		EXPECT_EQ(second.value()[0], to_bytes("cdefgh"));
	}
	...
	TEST(LineDelimitation, KeepsCarriageReturnInRecord) {
		...
		ASSERT_TRUE(first.has_value());
		ASSERT_EQ(first.value().size(), 2U);
		EXPECT_EQ(first.value()[0], to_bytes("ab\r"));
		EXPECT_EQ(first.value()[1], to_bytes("cde\r"));
	}

	TEST(LineDelimitation, EmptyChunkWithoutFinalEmitsNothing) {
		LineDelimitation delimiter;
		auto result = delimiter.delimit(to_bytes(""), false);
		ASSERT_TRUE(result.has_value());
	...
	TEST(LineDelimitation, EmptyChunkWithFinalEmitsNothing) {
		LineDelimitation delimiter;
		auto result = delimiter.delimit(to_bytes(""), true);
		ASSERT_TRUE(result.has_value());
		EXPECT_TRUE(result.value().empty());
	}
	...
	TEST(LineDelimitation, ConsecutiveEmptyLineEmitEmptyRecords) {
		...
		auto result = delimiter.delimit(to_bytes("a\n\n\nb\n"), true);
		ASSERT_TRUE(result.has_value());
		...
		EXPECT_EQ(records[3], to_bytes("b"));
	}
} // namespace cpe
